...
If the optional `blackList = TRUE` and the name of a reference genome is passed as input through `genome` (e.g. 'hg19'), `epigraHMMDataSetFromBam` will fetch the curated [ENCODE blacklist tracks](https://sites.google.com/site/anshulkundaje/projects/blacklists) from the Bioconductor package [GreyListChIP](https://bioconductor.org/packages/release/bioc/html/GreyListChIP.html). Current available genomes are ce10, dm3, hg19, hg38, and mm10. If `blackList` is a *GRanges* object, the function will discard any genomic coordinate overlapping regions from `blackList`
...
epigraHMM can also be used for genomic segmentation analyses. In this context, different experimental conditions in the argument `colData` are represented by different epigenomic marks, and differential peaks called by epigraHMM represent differential combinatorial chromatin states. In the example below, we utilize the histone modification marks H3K27me3 and H3K4me3 to find regions of differential enrichment. To speed up the computing time, I utilize a window size of 1000 base pairs.

```{r data_peak_segmentation,message = FALSE}
# Creating input for epigraHMM
bamFiles <- system.file(package="chromstaRData","extdata","euratrans",
                        c("lv-H3K27me3-SHR-male-bio2-tech1.bam",
                          "lv-H3K27me3-SHR-male-bio3-tech1.bam",
                          "lv-H3K4me3-SHR-male-bio2-tech1.bam",
                          "lv-H3K4me3-SHR-male-bio3-tech1.bam"))

colData <- data.frame(condition = c('H3K27me3','H3K27me3','H3K4me3','H3K4me3'),
                      replicate = c(1,2,1,2))

# Creating epigraHMM object
object_segmentation <- epigraHMMDataSetFromBam(bamFiles = bamFiles,
                                               colData = colData,
                                               genome = 'rn4',
                                               windowSize = 1000,
                                               gapTrack = TRUE)
...
# Normalizing counts
object_segmentation <- normalizeCounts(object_segmentation,
                                       control = controlEM())

# Initializing epigraHMM
object_segmentation <- initializer(object_segmentation,controlEM())

# Differential peak calling
object_segmentation <- epigraHMM(object = object_segmentation,
                                 control = controlEM(),
                                 type = 'differential',
                                 dist = 'nb')
```
...
```{r peaks_segmentation}
peaks_segmentation <- callPeaks(object = object_segmentation)
peaks_segmentation
```
